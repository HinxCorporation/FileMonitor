...
class DlistWorker:
    def __init__(self, dlist_path='dlist', sqlite_db_path='sys.db'):
        ...
        if self.worker_type == '':
            Log.logger.error('No worker type defined,stop')
            return
        sleep_tim_str = self.config_manager.get('sleep_time')[0]
        if sleep_tim_str is not None:
            self.sleep_time = int(sleep_tim_str)
        else:
            self.sleep_time = 10
...
if __name__ == '__main__':
    ...
    if worker.running:
        ...
        # 测试内容如下
        # {"op": "insert", "is_file": false, "path": "F:\\Root\\volume-Files\\History_01","cid": "000"}
        # {"op": "update", "is_file": true, "path": "F:\\Root\\volume-Files\\History_01\\table.xlsx", "cid": "000"}

        worker.i_process_file(test_file)
        Log.logger.info('Debug finished.')
    else:
        Log.logger.error('Dlist worker not running. exit')
